...
const static int8 dY[PATHFINDING_POSSIBLE_DIRECTIONS] = { 0, 0, -1, 1 };
const static int8 dX[PATHFINDING_POSSIBLE_DIRECTIONS] = { -1, 1, 0, 0 };

Queue *Pathfinding_find_shortest_path_bfs(Point *start, Point *end, uint8[][] maze)
{
	...
	{
		uint8 visited[PATHFINDING_MAZE_HEIGHT][PATHFINDING_MAZE_WIDTH];
		Node *current = Queue_pop(q);
		Point *point = current->value;
		for (uint8 i = 0; i < PATHFINDING_POSSIBLE_DIRECTIONS; ++i)
		{
			uint8 newX = point->x + dX[i];
			uint8 newY = point->y + dY[i];
