...
Stack *Pathfinding_find_shortest_path_bfs(Point *start, Point *end, uint8_t maze[PATHFINDING_MAZE_HEIGHT][PATHFINDING_MAZE_WIDTH])
{
	...
	while (!Queue_is_empty(q))
	{
		...
		if (!Point_equal(start, current_point))
		{
...
PathfindingRoute *Pathfinding_generate_route_to_food(Stack *shortest_path, MazeDirections starting_direction, uint8_t maze[PATHFINDING_MAZE_HEIGHT][PATHFINDING_MAZE_WIDTH])
{
	...
	while (!Stack_is_empty(shortest_path))
	{
		...
		if (Stack_is_empty(shortest_path))
		{
			...
			finalDistance = Pathfinding_calculate_point_spacing(current_direction, current_point, last_intersection_point);
	...
	PathfindingRoute *route = Pathfinding_route_construct(turns, current_direction, finalDistance + 1);
	return route;
}
